CREATE OR REPLACE function get_mod_desp_parents_economica(codigo_org varchar,codigo_eco varchar) RETURNS boolean AS $$
            DECLARE
                ok boolean=TRUE;
            ...
            END LOOP;
             WHILE (ok=true) LOOP
               FOR dados IN (SELECT code,name,parent_id FROM account_analytic_account WHERE id=parent_atual) LOOP
                   ...
                   IF dados.code NOT IN (SELECT PO.codigo_eco FROM sncp_modificacao_imprimir_despesa AS PO WHERE PO.codigo_eco=dados.code
                   AND PO.codigo_org=$1) THEN
                    INSERT INTO sncp_modificacao_imprimir_despesa(codigo_eco,codigo_org,name,linha) VALUES (dados.code,$1,dados.name,linha);
                    parent_atual=dados.parent_id;
