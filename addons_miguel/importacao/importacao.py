...
class importacao(osv.Model):
    ...
    def product_template(self, cr, caminho_ficheiro):
        ...
        if x is True:
            ...
            cr.execute("""
            COPY product_template(id,list_price,uom_id,cost_method,categ_id,name,sale_ok,company_id,uom_po_id,type,
            supply_method,procure_method,purchase_ok,standard_price)
    ...
    def product(self, cr, caminho_ficheiro):
        ...
        if x is True:
            ...
            cr.execute("""COPY product_product(id,default_code,name_template,active,valuation,product_tmpl_id)
            FROM '%s' DELIMITER ',' CSV HEADER;""" % ficheiro)
            cr.execute("""
            SELECT MAX(id)
