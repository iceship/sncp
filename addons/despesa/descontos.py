...
class sncp_despesa_descontos_retencoes(osv.Model):
    ...
    def get_ean13(self, cr, uid, ids, fields, arg, context):
        ...
        for desc_ret_id in ids:
            obj = db_sncp_despesa_descontos_retencoes.browse(cr, uid, desc_ret_id)
            ean13[desc_ret_id] = obj.cod_contab_id.ean13 or None
        return ean13

    def get_conta_code(self, cr, uid, ids, fields, arg, context):
        conta_code = {}
        db_sncp_despesa_descontos_retencoes = self.pool.get('sncp.despesa.descontos.retencoes')
        for desc_ret_id in ids:
            obj = db_sncp_despesa_descontos_retencoes.browse(cr, uid, desc_ret_id)
            conta_code[desc_ret_id] = obj.cod_contab_id.conta_id.code or None
        return conta_code

    def get_conta_name(self, cr, uid, ids, fields, arg, context):
        conta_name = {}
        db_sncp_despesa_descontos_retencoes = self.pool.get('sncp.despesa.descontos.retencoes')
        for desc_ret_id in ids:
            obj = db_sncp_despesa_descontos_retencoes.browse(cr, uid, desc_ret_id)
            conta_name[desc_ret_id] = obj.cod_contab_id.conta_id.name or None
        return conta_name

    def get_organica_code(self, cr, uid, ids, fields, arg, context):
        organica_code = {}
        db_sncp_despesa_descontos_retencoes = self.pool.get('sncp.despesa.descontos.retencoes')
        for desc_ret_id in ids:
            obj = db_sncp_despesa_descontos_retencoes.browse(cr, uid, desc_ret_id)
            organica_code[desc_ret_id] = obj.cod_contab_id.organica_id.code or None
        return organica_code

    def get_organica_name(self, cr, uid, ids, fields, arg, context):
        organica_name = {}
        db_sncp_despesa_descontos_retencoes = self.pool.get('sncp.despesa.descontos.retencoes')
        for desc_ret_id in ids:
            obj = db_sncp_despesa_descontos_retencoes.browse(cr, uid, desc_ret_id)
            organica_name[desc_ret_id] = obj.cod_contab_id.organica_id.name or None
        return organica_name

    def get_economica_code(self, cr, uid, ids, fields, arg, context):
        economica_code = {}
        db_sncp_despesa_descontos_retencoes = self.pool.get('sncp.despesa.descontos.retencoes')
        for desc_ret_id in ids:
            obj = db_sncp_despesa_descontos_retencoes.browse(cr, uid, desc_ret_id)
            economica_code[desc_ret_id] = obj.cod_contab_id.economica_id.code or None
        return economica_code

    def get_economica_name(self, cr, uid, ids, fields, arg, context):
        economica_name = {}
        db_sncp_despesa_descontos_retencoes = self.pool.get('sncp.despesa.descontos.retencoes')
        for desc_ret_id in ids:
            obj = db_sncp_despesa_descontos_retencoes.browse(cr, uid, desc_ret_id)
            economica_name[desc_ret_id] = obj.cod_contab_id.economica_id.name or None
        return economica_name
    ...
    _columns = {
        ...
        'ean': fields.function(get_ean13, arg=None, method=False, type="char",
                               string=u'Código EAN 13', store=True),
        'conta_code': fields.function(get_conta_code, arg=None, method=False, type="char",
                                      string=u'Patrimonial', store=True),

        'conta_name': fields.function(get_conta_name, arg=None, method=False, type="char",
                                      store=True),

        'organica_code': fields.function(get_organica_code, arg=None, method=False, type="char",
                                         string=u'Orgânica', store=True),
        'organica_name': fields.function(get_organica_name, arg=None, method=False, type="char",
                                         string=u'Orgânica', store=True),
        'economica_code': fields.function(get_economica_code, arg=None, method=False, type="char",
                                          string=u'Económica', store=True),
        'economica_name': fields.function(get_economica_name, arg=None, method=False, type="char",
                                          string=u'Económica', store=True),
        # -----------------------------FIM------------------------------
