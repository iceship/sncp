...
class sncp_despesa_requisicoes_converter_wizard(osv.Model):
    ...
    def continuar(self, cr, uid, ids, context=None):
        ...
        # 6. LOCALIZAÇÃO
        localizacao_id = obj.goc_id.armazem_id.wh_input_stock_loc_id.id
        ...
        for linha in linhas:
            ...
            """ % (obj.goc_id.fornecedor_id.id, produto.product_tmpl_id.id))
            ...
            res_id = u'product.template,' + unicode(produto.product_tmpl_id.id)
            cr.execute("""
            SELECT value_float
            FROM ir_property
            ...
            if preco_standard is None or preco_standard[0] <= 0.0:
                raise osv.except_osv(_(u'Aviso'), _(u'O preço de custo do produto ' +
                                                    u'['+unicode(produto.default_code)+u'] ' +
                                                    unicode(produto.name_template)))

            rodape = {'product_uom': produto.product_tmpl_id.uom_id.id,
                      'order_id': ordem_compra_id,
                      'price_unit': preco_standard[0],
                      'product_qty': linha[1],
                      'partner_id': parceiro_id,
                      'invoiced': False,
