...
class sncp_despesa_requisicoes(osv.Model):
    ...
    def aprovar(self, cr, uid, ids, context):
        ...
        cr.execute("""UPDATE sncp_despesa_requisicoes_linhas SET state = 'aprovd'
    ...
    def rejeitar(self, cr, uid, ids, context):
        ...
        # linhas
        cr.execute("""UPDATE sncp_despesa_requisicoes_linhas SET state = 'rejeit'
                      WHERE req_id = %d""" % ids[0])
        return {'type': 'ir.actions.client', 'tag': 'reload'}
    ...
    def recuperar(self, cr, uid, ids, context):
        ...
        # linhas
        cr.execute("""UPDATE sncp_despesa_requisicoes_linhas SET state = 'draft'
                      WHERE req_id = %d""" % ids[0])
        return {'type': 'ir.actions.client', 'tag': 'reload'}
    ...
    def criar_entrega(self, cr, uid, ids, context):
        ...
        for linha in linhas:
            ...
            values = {
                ...
                'product_id': linha[2],
                'product_uom': linha[0],
                'product_uom_qty': qt_dif_req_sat,
                'product_uos': None,
                'location_id': obj.armazem_req_id.wh_output_stock_loc_id.id,
    ...
    def entregar(self, cr, uid, ids, context=None):
        ...
        for stock_move in db_stock_move.browse(cr, uid, stock_move_ids):
            ...
            if stock_move.product_uom_qty > qt_dif_req_sat:
        ...
        for linha_id in linhas_ids:
            ...
            quant_satis = linha.quant_sat + stock_move.product_uom_qty
...
class sncp_despesa_requisicoes_linhas(osv.Model):
    ...
    def get_item_details(self, cr, uid, ids, item_id):
        ...
        preco_standard = cr.fetchone()

        cr.execute(""" SELECT uom_id FROM product_template
                       WHERE id = %d""" % item_id)
        result = cr.fetchone()
        if result is None and preco_standard is None:
            result = [0.0, False]
            taxa = 0.0
            return [result[0], result[1], taxa]
        elif preco_standard is None or preco_standard[0] <= 0.0:
            raise osv.except_osv(_(u'Aviso'), _(u'O preço de custo deste artigo não está definido corretamente.'))
        elif result[0] is None or result[0] is False:
        ...
        return [preco_standard[0], result[0], taxa]
