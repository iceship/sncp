...
END LOOP;
 WHILE (ok=true) LOOP
   FOR dados IN (SELECT code,name,parent_id FROM account_analytic_account WHERE id=parent_atual) LOOP
	   ...
	   IF dados.code NOT IN (SELECT PO.codigo_org FROM sncp_orcamento_imprimir_despesa AS PO WHERE PO.codigo_org=dados.code 
	   AND PO.codigo_eco='') THEN
		INSERT INTO sncp_orcamento_imprimir_despesa(codigo_eco,codigo_org,name,linha,orcamento_id) VALUES ('',dados.code,dados.name,linha,$2);
		parent_atual=dados.parent_id;
